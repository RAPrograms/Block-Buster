...
<Modal show={currentQuestion != undefined}>
    <form data-team={currentTeam} on:submit|preventDefault={Guess}>
        ...
        <input type="text" use:focus placeholder='Starts with an "{currentQuestion?.answer[0].toUpperCase()}"' on:input={() => {error = false}} class:error={error} bind:value={guessValue} required>
